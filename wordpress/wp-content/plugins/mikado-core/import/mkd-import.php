...
class HoshiMikadoImport {

    public $message = "";
    public $attachments = false;
    function HoshiMikadoImport() {
    ...
    function init_mkd_import() {
        if(isset($_REQUEST['import_option'])) {
            ...
            if($import_option == 'content'){
                ...
                $this->import_menus('menus.txt');
                $this->import_settings_pages('settingpages.txt');
                $this->message = esc_html__("Content imported successfully", "hoshi");
            }
        }
    }
    ...
    public function import_content($file){
        ...
        if(is_wp_error($returned_value)){
            $this->message = esc_html__("An Error Occurred During Import", "hoshi");
        }
        else {
            $this->message = esc_html__("Content imported successfully", "hoshi");
        }
    ...
    public function import_widgets($file, $file2){
        ...
        $this->message = esc_html__("Widgets imported successfully", "hoshi");
    }
    ...
    public function import_custom_sidebars($file){
        ...
        $this->message = esc_html__("Custom sidebars imported successfully", "hoshi");
    }

    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'mkd_options_hoshi', $options);
        $this->message = esc_html__("Options imported successfully", "hoshi");
    }
    ...
    public function file_options($file){
        ...
        /*if ( file_exists($file_for_import) ) {
            ...
            $this->message = esc_html__("File doesn't exist", "hoshi");
        }*/
    ...
    function mkd_admin_import() {
        if (mkd_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                'hoshi_mikado_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                array($hoshi_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'hoshi_mikado_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'hoshi_mikado_enqueue_admin_styles');
...
function mkd_init_import_object(){
    global $hoshi_import_object;
    $hoshi_import_object = new HoshiMikadoImport();
}

add_action('init', 'mkd_init_import_object');

if(!function_exists('hoshi_mikado_dataImport')){
    function hoshi_mikado_dataImport(){
        global $hoshi_import_object;

        if ($_POST['import_attachments'] == 1)
            $hoshi_import_object->attachments = true;
        else
            $hoshi_import_object->attachments = false;

        $folder = "hoshi/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $hoshi_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_mkd_dataImport', 'hoshi_mikado_dataImport');
}

if(!function_exists('hoshi_mikado_widgetsImport')){
    function hoshi_mikado_widgetsImport(){
        global $hoshi_import_object;
        ...
        $hoshi_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_mkd_widgetsImport', 'hoshi_mikado_widgetsImport');
}

if(!function_exists('hoshi_mikado_optionsImport')){
    function hoshi_mikado_optionsImport(){
        global $hoshi_import_object;
        ...
        $hoshi_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_mkd_optionsImport', 'hoshi_mikado_optionsImport');
}

if(!function_exists('hoshi_mikado_otherImport')){
    function hoshi_mikado_otherImport(){
        global $hoshi_import_object;
        ...
        $hoshi_import_object->import_options($folder.'options.txt');
        $hoshi_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $hoshi_import_object->import_menus($folder.'menus.txt');
        $hoshi_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_mkd_otherImport', 'hoshi_mikado_otherImport');
}