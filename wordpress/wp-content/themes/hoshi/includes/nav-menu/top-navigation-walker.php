 ...
 */
if(!class_exists('HoshiMikadoTopNavigationWalker')) {
	class HoshiMikadoTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			// build html
			$output .= "\n".$indent.$out_div.'<ul '.hoshi_mikado_get_inline_style($menu_image_background).'>'."\n";
		}
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
			...
			if($item->hide == '') {
				...
				if(!empty($icon) && hoshi_mikado_get_meta_field_intersect('header_type', hoshi_mikado_get_page_id()) !== 'header-vertical') {
				...
				if(!empty($item->description) && hoshi_mikado_get_meta_field_intersect('header_type', hoshi_mikado_get_page_id()) !== 'header-vertical') {
