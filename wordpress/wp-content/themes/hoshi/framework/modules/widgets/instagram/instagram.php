...
class HoshiMikadoInstagramWidget extends HoshiMikadoWidget {
    protected $params;

    public function __construct() {
        parent::__construct(
            'mkd_instagram_widget',
            esc_html__('Mikado Instagram Widget', 'hoshi'),
            array('description' => esc_html__('Display instagram images', 'hoshi'))
        );
    ...
    protected function setParams() {
        $this->params = array(
            ...
            array(
                'name'  => 'tag',
                'type'  => 'textfield',
                'title' => esc_html__('Tag', 'hoshi')
            ),
            array(
                'name'  => 'number_of_photos',
                'type'  => 'textfield',
                'title' => esc_html__('Number of photos', 'hoshi')
            ),
            array(
                'name'    => 'number_of_cols',
                'type'    => 'dropdown',
                'title'   => esc_html__('Number of columns', 'hoshi'),
                'options' => array(
                    '2' => esc_html__('Two', 'hoshi'),
                    '3' => esc_html__('Three', 'hoshi'),
                    '4' => esc_html__('Four', 'hoshi'),
                    '6' => esc_html__('Six', 'hoshi'),
                    '9' => esc_html__('Nine', 'hoshi'),
                )
            ),
            array(
                'name'    => 'image_size',
                ...
                'options' => array(
                    'thumbnail'           => esc_html__('Small', 'hoshi'),
                    'low_resolution'      => esc_html__('Medium', 'hoshi'),
                    'standard_resolution' => esc_html__('Large', 'hoshi')
                )
            ),
            array(
                'name'  => 'transient_time',
                'type'  => 'textfield',
                'title' => esc_html__('Images Cache Time', 'hoshi')
            ),
        );
    }
                ...
                foreach($images_array as $image) { ?>
                    <li>
                        <a href="<?php echo esc_url($instagram_api->getHelper()->getImageLink($image)); ?>" target="_blank">
                            <?php echo hoshi_mikado_kses_img($instagram_api->getHelper()->getImageHTML($image, $image_size)); ?>
                        </a>
                    </li>
                <?php } ?>
            </ul>
