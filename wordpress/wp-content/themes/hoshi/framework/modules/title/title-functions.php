...
if (!function_exists('hoshi_mikado_get_title')) {
	/**
	 * Loads title area HTML
	 */
	function hoshi_mikado_get_title()
	{
		$id = hoshi_mikado_get_page_id();

		extract(hoshi_mikado_title_area_height());
		extract(hoshi_mikado_title_area_background());
		extract(hoshi_mikado_animated_gradient_overlay(hoshi_mikado_get_meta_field_intersect('title_gradient_overlay_color1',$id),hoshi_mikado_get_meta_field_intersect('title_gradient_overlay_color2',$id),hoshi_mikado_get_meta_field_intersect('title_gradient_overlay_color3',$id),hoshi_mikado_get_meta_field_intersect('title_gradient_overlay_color4',$id)));
		...
		if (($meta_temp = get_post_meta($id, "mkd_show_title_area_meta", true)) !== "") {
			...
			$show_title_area = hoshi_mikado_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
		}
		...
		if (($meta_temp = get_post_meta($id, "mkd_title_area_type_meta", true)) !== "") {
			...
			$type = hoshi_mikado_options()->getOptionValue('title_area_type');
		}
		...
		if (($meta_temp = get_post_meta($id, "mkd_title_area_enable_breadcrumbs_meta", true)) !== "") {
			...
			$enable_breadcrumbs = hoshi_mikado_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
		}
		...
		$parameters = array(
			...
			'show_animated_gradient_overlay'=> hoshi_mikado_get_meta_field_intersect('title_area_gradient_overaly_animation',$id) == 'yes' ? true : false,
		...
		$parameters = apply_filters('hoshi_mikado_title_area_height_params', $parameters);

		hoshi_mikado_get_module_template_part('templates/title', 'title', '', $parameters);
	}
}

if (!function_exists('hoshi_mikado_get_title_text')) {
	/**
	 * Function that returns current page title text. Defines hoshi_mikado_title_text filter
	 ...
	 */
	function hoshi_mikado_get_title_text()
	{
		...
		if (is_tag()) {
			...
			$title = single_term_title("", false) . esc_html__(' Tag', 'hoshi');
		...
		elseif (is_author()) {
			...
			$title = esc_html__('Author:', 'hoshi') . " " . get_the_author();
		...
		elseif (is_search()) {
			...
			$title = esc_html__('Search', 'hoshi');
		} //is current page 404?
		elseif (is_404()) {
			//is 404 title text set in theme options?
			if (hoshi_mikado_options()->getOptionValue('404_title') != "") {
				//get it from options
				$title = hoshi_mikado_options()->getOptionValue('404_title');
			} else {
				//get default 404 page title
				$title = esc_html__('404 - Page not found', 'hoshi');
			}
		}//is single post page
        elseif (is_singular('post')) {
            if (hoshi_mikado_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
                $title = get_the_title($id);
            } else {
                $title = esc_html__('Blog', 'hoshi');
            }
        }//is WooCommerce installed and is shop or single product page?
		elseif (hoshi_mikado_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
		...
		elseif (hoshi_mikado_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
			global $wp_query;
		...
		elseif (is_archive()) {
			$title = esc_html__('Archive', 'hoshi');
		} //current page is regular page
		else {
			$title = get_the_title($id);
		}

		$title = apply_filters('hoshi_mikado_title_text', $title);

		return $title;
	}
}

if (!function_exists('hoshi_mikado_title_text')) {
	/**
	 * Function that echoes title text.
	 *
	 * @see hoshi_mikado_get_title_text()
	 */
	function hoshi_mikado_title_text()
	{
		echo hoshi_mikado_get_title_text();
	}
}

if (!function_exists('hoshi_mikado_subtitle_text')) {
	/**
	 * Function that echoes subtitle text.
	 *
	 */
	function hoshi_mikado_subtitle_text()
	{
		$id = hoshi_mikado_get_page_id();
		$subtitle = '';
...
if (!function_exists('hoshi_mikado_custom_breadcrumbs')) {
	/**
	 ...
	 * @see is_front_page()
	 * @see is_category()
	 * @see hoshi_mikado_is_product_category()
	 * @see get_search_query()
	 */
	function hoshi_mikado_custom_breadcrumbs()
	{
		global $post, $wp_query;

		$output = "";
		$homeLink = esc_url(home_url('/'));
		$pageid = hoshi_mikado_get_page_id();
		$bread_style = "";
		...
		$delimiter = '<span class="mkd-delimiter" ' . hoshi_mikado_get_inline_style($bread_style) . '><span class="fa fa-caret-right"></span></span>'; // delimiter between crumbs
		$home = get_bloginfo('name'); // text for the 'Home' link
		$showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
		$before = '<span class="mkd-current" ' . hoshi_mikado_get_inline_style($bread_style) . '>'; // tag before the current crumb
		$after = '</span>'; // tag after the current crumb

		if (is_home() && !is_front_page()) {
			$output = '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a ' . hoshi_mikado_get_inline_style($bread_style) . ' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a ' . hoshi_mikado_get_inline_style($bread_style) . ' href="' . $homeLink . '">' . get_the_title($pageid) . '</a></div></div>';

		} elseif (is_home()) {
			$output = '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner">' . $before . $home . $after . '</div></div>';
		} elseif (is_front_page()) {
			if ($showOnHome == 1) {
				$output = '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a ' . hoshi_mikado_get_inline_style($bread_style) . ' href="' . $homeLink . '">' . $home . '</a></div></div>';
			}
		} else {

			$output .= '<div class="mkd-breadcrumbs"><div class="mkd-breadcrumbs-inner"><a ' . hoshi_mikado_get_inline_style($bread_style) . ' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

			if (is_category() || hoshi_mikado_is_product_category()) {
				...
				$output .= $before . esc_html__('Search results for ', 'hoshi') . '"' . get_search_query() . '"' . $after;

			} elseif (is_day()) {
				$output .= '<a ' . hoshi_mikado_get_inline_style($bread_style) . ' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
				$output .= '<a ' . hoshi_mikado_get_inline_style($bread_style) . ' href="' . get_month_link(get_the_time('Y'), get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
				...
				$output .= '<a ' . hoshi_mikado_get_inline_style($bread_style) . ' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
				if ($showCurrent == 1) {
				...
				while ($parent_id) {
					...
					$breadcrumbs[] = '<a ' . hoshi_mikado_get_inline_style($bread_style) . ' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
				...
				$output .= $before . esc_html__('Posts tagged ', 'hoshi') . '"' . single_tag_title('', false) . '"' . $after;

			} elseif (is_author()) {
				global $authordata;
				$output .= $before . esc_html__('Articles posted by ', 'hoshi') . $authordata->display_name . $after;
			...
			if (get_query_var('paged')) {
				...
				$output .= $before . " (" . esc_html__('Page', 'hoshi') . ' ' . get_query_var('paged') . ")" . $after;
...
if (!function_exists('hoshi_mikado_get_title_area_height')) {

	/**
	 * Function that returns title height
	 **/
	function hoshi_mikado_get_title_area_height()
	{

		$id = hoshi_mikado_get_page_id();
		$title_type = hoshi_mikado_get_meta_field_intersect('title_area_type',$id);
        $title_size = hoshi_mikado_get_meta_field_intersect('title_area_text_size', $id);
		...
		} elseif (hoshi_mikado_options()->getOptionValue('title_area_height') !== '') {
			$title_height = hoshi_mikado_options()->getOptionValue('title_area_height');
		}

		return apply_filters('hoshi_mikado_title_area_height', $title_height);
	}
}

if (!function_exists('hoshi_mikado_get_title_content_padding')) {
	/**
	 * Function that returns title content pading
	 **/

	function hoshi_mikado_get_title_content_padding()
	{
		return apply_filters('hoshi_mikado_title_content_padding', 0);
	}
}


if (!function_exists('hoshi_mikado_title_area_height')) {
	/**
	 * Function that returns title height and padding to be applied in template
	 **/

	function hoshi_mikado_title_area_height()
	{
		$id = hoshi_mikado_get_page_id();
		$title_height_and_padding = array();
		$title_height = hoshi_mikado_get_title_area_height();
		$header_height_padding = hoshi_mikado_get_title_content_padding();
		...
		if (($meta_temp = get_post_meta($id, "mkd_title_area_background_image_responsive_meta", true)) != "") {
			...
			$is_img_responsive = hoshi_mikado_options()->getOptionValue('title_area_background_image_responsive');
		}

		if (($meta_temp = get_post_meta($id, "mkd_title_area_vertial_alignment_meta", true)) !== '') {
			$title_vertical_alignment = $meta_temp;
		} else {
			$title_vertical_alignment = hoshi_mikado_options()->getOptionValue('title_area_vertial_alignment');;
		}
...
if (!function_exists('hoshi_mikado_title_area_background')) {
	/**
	 * Function that returns title background style be applied in template
	 **/

	function hoshi_mikado_title_area_background()
	{
		$id = hoshi_mikado_get_page_id();
		$show_title_img = true;
		...
		if (($meta_temp = get_post_meta($id, "mkd_title_area_background_color_meta", true)) != "") {
			...
			$background_color = hoshi_mikado_options()->getOptionValue('title_area_background_color');
		}
		...
		if (($meta_temp = get_post_meta($id, "mkd_title_area_background_image_meta", true)) != "") {
			...
			$background_image = hoshi_mikado_options()->getOptionValue('title_area_background_image');
		}

		//check for background image width
		$background_image_width = "";
		if ($background_image !== '') {
			$background_image_width_dimensions_array = hoshi_mikado_get_image_dimensions($background_image);
