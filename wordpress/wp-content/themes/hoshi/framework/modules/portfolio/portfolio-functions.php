...
if(!function_exists('hoshi_mikado_single_portfolio')) {
	/**
	 * Loads holder template for portfolio single
	 */
	function hoshi_mikado_single_portfolio() {
		$portfolio_template = hoshi_mikado_get_portfolio_single_type();
		...
		if($portfolio_template == 'gallery') {
			$params['holder_class'][] = 'mkd-portfolio-gallery-'.hoshi_mikado_options()->getOptionValue('portfolio_single_numb_columns');
		}

		hoshi_mikado_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
	}
}

if(!function_exists('hoshi_mikado_get_portfolio_single_type')) {
	/**
	 * Returns template of current portfolio item
	 *
	 * @return bool|mixed|void
	 */
	function hoshi_mikado_get_portfolio_single_type() {
		return hoshi_mikado_get_meta_field_intersect('portfolio_single_template');
	}
}

if(!function_exists('hoshi_mikado_get_portfolio_single_media')) {
	/**
	 * Returns a media array of current portfolio item
	 *
	 * @return array
	 */
	function hoshi_mikado_get_portfolio_single_media() {
		...
		if(is_array($videos) && count($videos)) {
			usort($videos, 'hoshi_mikado_compare_portfolio_videos');
			foreach($videos as $video) {
				...
				if(!empty($video['portfoliovideotype'])) {
					...
					$media['video_url']   = hoshi_mikado_portfolio_get_video_url($video);
...
if(!function_exists('hoshi_mikado_portfolio_get_video_url')) {
	/**
	 ...
	 * @return array|string
	 */
	function hoshi_mikado_portfolio_get_video_url($video) {
...
if(!function_exists('hoshi_mikado_portfolio_get_media_html')) {
	/**
	 * Loads template for current portfolio item media
	 *
	 * @param $media
	 */
	function hoshi_mikado_portfolio_get_media_html($media) {
		global $wp_filesystem;
		$params = array();

		//For adding image overlay in gallery template type
		$params['gallery'] = (hoshi_mikado_get_portfolio_single_type() == 'gallery') ? true : false;

		if($media['type'] == 'image') {
			$params['lightbox'] = hoshi_mikado_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
		...
		if(in_array($media['type'], array('youtube', 'vimeo'))) {
			$params['lightbox'] = hoshi_mikado_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
		...
		hoshi_mikado_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
	}
}

if(!function_exists('hoshi_mikado_compare_portfolio_videos')) {
	/**
	 ...
	 */
	function hoshi_mikado_compare_portfolio_videos($a, $b) {
...
if(!function_exists('hoshi_mikado_compare_portfolio_options')) {
	/**
	 ...
	 * @return int result of comparison
	 */
	function hoshi_mikado_compare_portfolio_options($a, $b) {
...
if(!function_exists('hoshi_mikado_portfolio_get_info_part')) {
	/**
	 * Loads portfolio info item based on passed param
	 *
	 * @param $part
	 */
	function hoshi_mikado_portfolio_get_info_part($part) {
		$portfolio_template = hoshi_mikado_get_portfolio_single_type();

		hoshi_mikado_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template);
	}
}

if(!function_exists('hoshi_mikado_portfolio_get_single_navigation')) {
	/**
	 *
	 */
	function hoshi_mikado_portfolio_get_single_navigation() {
		$params = array();

		$in_same_term = hoshi_mikado_options()->getOptionValue('portfolio_single_nav_same_category') == 'yes';
		...
		hoshi_mikado_get_module_template_part('templates/single/parts/navigation', 'portfolio', '', $params);
	}
}

if(!function_exists('hoshi_mikado_get_ptf_masonry_layout')){
	function hoshi_mikado_get_ptf_masonry_layout(){
		$html = '';
		...
		if($image_gallery_val !== ""){
			...
			if ( isset($image_gallery_array) && count($image_gallery_array) !== 0) {
				...
				foreach($image_gallery_array as $image_gallery_id){
					...
					$image_size_value = 'hoshi_mikado_landscape';
					if($image_size){
						switch($image_size){
							case 'large_width_height' :
								$image_size_class = 'mkd-ptf-img-large-height-width';
								$image_size_value = 'hoshi_mikado_large_width_height';
								break;
							case 'large_height' :
								$image_size_class = 'mkd-ptf-img-large-height';
								$image_size_value = 'hoshi_mikado_large_height';
								break;
							default:
								$image_size_class = 'default';
								$image_size_value = 'hoshi_mikado_large_width_height';
