<?php
namespace Hoshi\Modules\Separator;
...
class Separator implements ShortcodeInterface {
    ...
    public function vcMap() {
        ...
        vc_map(
            array(
                'name'                    => esc_html__('Separator', 'hoshi'),
                'base'                    => $this->base,
                'category'                => 'by MIKADO',
                ...
                'params'                  => array(
                    array(
                        'type'        => 'textfield',
                        'heading'     => esc_html__('Extra class name', 'hoshi'),
                        'param_name'  => 'class_name',
                        'value'       => '',
                        'description' => esc_html__('Style particular content element differently - add a class name and refer to it in custom CSS.', 'hoshi')
                    ),
                    array(
                        'type'        => 'dropdown',
                    ...
                    array(
                        'type'       => 'dropdown',
                        'heading'    => esc_html__('Border Style', 'hoshi'),
                        'param_name' => 'border_style',
                        'value'      => array(
                            esc_html__('Default', 'hoshi') => '',
                            esc_html__('Dashed', 'hoshi')  => 'dashed',
                            esc_html__('Solid', 'hoshi')   => 'solid',
                            esc_html__('Dotted', 'hoshi')  => 'dotted'
                        )
                    ),
                    array(
                        'type'        => 'textfield',
                    ...
                    array(
                        'type'        => 'textfield',
                        'heading'     => esc_html__('Thickness (px)', 'hoshi'),
                    ...
                    array(
                        'type'        => 'textfield',
                        'heading'     => esc_html__('Top Margin', 'hoshi'),
                    ...
                    array(
                        'type'       => 'textfield',
                        'heading'    => esc_html__('Bottom Margin', 'hoshi'),
    ...
    public function render($atts, $content = null) {
        ...
        $html = hoshi_mikado_get_shortcode_module_template_part('templates/separator-template', 'separator', '', $params);
    ...
    private function getSeparatorStyle($params) {
        ...
        if($params['width'] !== '') {
            if(hoshi_mikado_string_ends_with($params['width'], '%') || hoshi_mikado_string_ends_with($params['width'], 'px')) {
        ...
        if($params['top_margin'] !== '') {
            if(hoshi_mikado_string_ends_with($params['top_margin'], '%') || hoshi_mikado_string_ends_with($params['top_margin'], 'px')) {
        ...
        if($params['bottom_margin'] !== '') {
            if(hoshi_mikado_string_ends_with($params['bottom_margin'], '%') || hoshi_mikado_string_ends_with($params['bottom_margin'], 'px')) {
