...
class HoshiMikadoLike
{
	...
	private function __construct()
	{
		...
		add_action('wp_ajax_hoshi_mikado_like', array($this, 'ajax'));
		add_action('wp_ajax_nopriv_hoshi_mikado_like', array($this, 'ajax'));
	}
	...
	public function like_post($post_id, $action = 'get', $type = '')
	{
		...
		switch ($action) {
			...
			case 'get':
				...
				$like_label = $like_count !== '1' ? esc_html__('likes','hoshi') : esc_html__('like','hoshi');
			...
			case 'update':
				$like_count = get_post_meta($post_id, '_mkd-like', true);
				$like_label = $like_count !== '0' ? esc_html__('likes','hoshi') : esc_html__('like','hoshi');
				...
				if ($type != 'portfolio_list') {
					$return_value = '<i class="fa fa-thumbs-up" aria-hidden="true"></i><span class = "mkd-like-number">' . $like_count . '</span><span>' . $like_label . '</span>';
	...
	public function add_like()
	{
		...
		$title = esc_html__('Like this', 'hoshi');
		if (isset($_COOKIE['mkd-like_' . $post->ID])) {
			$class = 'mkd-like liked';
			$title = esc_html__('You already like this!', 'hoshi');
		}
	...
	public function add_like_portfolio_list($portfolio_project_id)
	{
		...
		return '<a href="#" class="' . $class . '" data-type="portfolio_list" id="mkd-like-' . $portfolio_project_id . '-' . $rand_number . '" title="' . $title . '">'. esc_html__('Like', 'hoshi') .'</a>';
	}
...
global $hoshi_like;
$hoshi_like = HoshiMikadoLike::get_instance();
