...
class HoshiMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions   = HoshiMikadoOptions::get_instance();
		$this->mkdMetaBoxes = HoshiMikadoMetaBoxes::get_instance();
	}
	...
	public function setSkin(HoshiMikadoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class HoshiMikadoSkinManager
 *
...
/**
 * Class HoshiMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class HoshiMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	/**
	 * Loads template part with params. Uses hoshi_mikado_find_template_path function which is child theme friendly
	 *
	...
	public function loadTemplatePart($template, $params = array()) {
		...
		if($template !== '') {
			include(hoshi_mikado_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo hoshi_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class HoshiMikadoOptions {
	...
	public function getOptionValue($key) {
		...
		if(array_key_exists($key, $hoshi_options)) {
			return $hoshi_options[$key];
...
/*
   Class: HoshiMikadoAdminPage
   A class that initializes Mikado Admin Page
*/

class HoshiMikadoAdminPage implements iHoshiMikadoLayoutNode {
	...
	function __construct($slug = "", $title = "", $icon = "") {
		$this->layout  = array();
		$this->factory = new HoshiMikadoFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild(iHoshiMikadoRender $child) {
		$child->render($this->factory);
	}
}

/*
   Class: HoshiMikadoMetaBoxes
...
/*
   Class: HoshiMikadoMetaBox
   A class that initializes Mikado Meta Box
*/

class HoshiMikadoMetaBox implements iHoshiMikadoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $hoshi_Framework->getSkin()->getSkinLabel().' '.$label;
	}
}

global $hoshi_Framework;

$hoshi_Framework = HoshiMikadoFramework::get_instance();
$mkdSkinManager                 = new HoshiMikadoSkinManager();
$hoshi_Framework->setSkin($mkdSkinManager->getSkin());
